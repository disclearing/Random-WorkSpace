...
public class Grappler extends Kit {
	...
	public Grappler() {
		...
		setItens(new ItemBuilder().material(Material.LEASH).name(getItemColor() + "Kit " + getName()).build());
	}
	...
	public void onPlayerItemHeld(PlayerItemHeldEvent event) {
		if (hooks.containsKey(event.getPlayer())) {
			((Cordinha) hooks.get(event.getPlayer())).remove();
	...
	public void onFall(EntityDamageEvent event) {
		...
		if (event.getCause() == EntityDamageEvent.DamageCause.FALL) {
			...
			if ((hooks.containsKey(player)) && (((Cordinha) hooks.get(player)).isHooked())
	...
	public void usar(PlayerInteractEvent e) {
		...
		if ((p.getItemInHand().getType().equals(Material.LEASH)) && (containsHability(p))) {
			...
			if ((e.getAction() == Action.LEFT_CLICK_AIR) || (e.getAction() == Action.LEFT_CLICK_BLOCK)) {
				...
				nmsHook.move(p.getLocation().getDirection().getX() * 2.5D, p.getLocation().getDirection().getY() * 2.5D,
						p.getLocation().getDirection().getZ() * 2.5D);
				...
				double v_x = (1.0D + 0.07000000000000001D * t)
						* (((Cordinha) hooks.get(p)).getBukkitEntity().getLocation().getX() - p.getLocation().getX())
						/ t;
				double v_y = (1.0D + 0.03D * t)
						* (((Cordinha) hooks.get(p)).getBukkitEntity().getLocation().getY() - p.getLocation().getY())
						/ t;
				double v_z = (1.0D + 0.07000000000000001D * t)
						* (((Cordinha) hooks.get(p)).getBukkitEntity().getLocation().getZ() - p.getLocation().getZ())
						/ t;
	...
	protected void clean(Player player) {
		if (hooks.containsKey(player)) {
			((Cordinha) hooks.get(player)).remove();
			hooks.remove(player);
		}

		if (leftClickGrappler.containsKey(player)) {
			leftClickGrappler.remove(player);
		}

		if (rightClickGrappler.containsKey(player)) {
